...
trait AppLogger {
  ...
  def getIncompleteTasks(batch_id: Long): mutable.ListBuffer[LogEntry]

  def getCompletedTasks(batch_id: Long): ListBuffer[String]
...
class LogEntryBuilder {
  ...
  def build(): LogEntry = {
    ...
    if (batch_id > 0)
      loga.log.put("BATCH_ID", batch_id + "")
...
class FileBasedLogger(val loggerBaseLocation: String) extends AppLogger {
  ...
  override def getIncompleteTasks(batch_id: Long): mutable.ListBuffer[LogEntry] = {
    null
  }

  override def getCompletedTasks(batch_id: Long): ListBuffer[String] = {
  ...
  override def getLatestBatchId(): Long = {
    1L
  }
